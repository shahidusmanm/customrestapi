...
Building the container can be done very easily through `docker build`. The tricky bit is to make sure the container is tagged properly so that it can be pushed to Google's Artifact Registry (from where we'll be loading it onto our container instance).

To do that you need to make sure that the tag or name of the image is in a specific format i.e. `<gcp_region>-docker.pkg.dev/<gcp_project_name>/<repository_name>/<container_name>:<tag>`

For our app, we used the following command (We've omitted some sensitive parts of the command)

```
docker build . --tag=<gcp_region>-docker.pkg.dev/<gcp_project_name>/flaskapp/cloudflaskappdeployment_gcp:v5
```
...
```
docker push <gcp_region>-docker.pkg.dev/<gcp_project_name>/flaskapp/cloudflaskappdeployment_gcp:v5  
```   
...
We want to allow all traffic because this container should be accessible from everywhere.  We don't want to have any unwanted restrictions on incoming requests.

![Ingress](images/ingress.png)

We also don't want to use Google's authentication system.  

![Authentication](images/authenthication.png)

A few more changes to make here.  We need to change this container board to 5000 because that's what our Flask app is listening on by default. This bit tells our container that any request you get on port 80 needs to be forwarded to port 5000.

![Container Port](images/container-port.png)

We are going to reduce the memory capacity because we don't need 512MB and we are going to keep the # of CPU cores to 1.
...
PS. We will shut the container down by the end of this project so the URL may not be available to you after April 2022.

![Postman](images/postman.png)

## Authentication
To use the app, you first need to login with your registered email and password through the /login endpoint. The authentication endpoint checks your credentials against a hashed version stored in the database and returns a session key if the credentials are correct.

You need to send this session key as part of the `user_api_key` body param with every HTTP Request you make. For every request, the api authenticates your session key and provides you with the requested resource.
